...
async function setupPinAndUsername(
  pin: string,
  username: string,
  driver: WebDriver,
) {
...
function runTests(browser: string) {
  describe(`${browser} tests`, () => {
    ...
    it("Should back in previous page", async () => {
      await driver.findElement(By.id("student-button")).click();
      const headingText = await driver
        .findElement(By.id("student-join-heading"))
        .getText();
      ...
      describe("Main text of teacher option", () => {
        it("See the main text of button", async () => {
          ...
          const msg = await driver
            .findElement(By.id("text-professor"))
            .getText();
