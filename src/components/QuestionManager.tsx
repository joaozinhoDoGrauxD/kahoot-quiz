...
export default function QuestionManager({
  ...
  return (
    <div className="bg-[#141416] p-6 rounded-xl border border-[#27272A] mt-6">
      <h2 className="text-xl font-bold mb-4 text-[#E4E4E7]">
        Adicionar Questão
      </h2>
      <input
        id="texto-pergunta"
      ...
      >
        <option id="multiple" value="multiple_choice">
          Múltipla Escolha (5 opções)
        </option>
        <option id="boolValue" value="true_false">
          Certo ou Errado
        </option>
        <option id="correspondence" value="match">
          Correspondência
        </option>
      </select>
      ...
      {type === "multiple_choice" && (
        <div className="mb-4 space-y-2">
          ...
          <p className="text-sm text-gray-500">
            Selecione o rádio para a resposta correta.
          </p>
        </div>
      )}
      ...
      <button
        onClick={add}
        className="bg-[#D4AF37] text-black font-bold px-6 py-2 rounded-lg w-full mt-2 hover:bg-opacity-90"
      >
        Salvar Questão
      </button>
    </div>
