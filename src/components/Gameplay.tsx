...
export default function Gameplay({
  ...
  const submitAnswer = async (answer: string | Record<number, string>) => {
    ...
    if (isCorrect) {
      ...
      pointsEarned = Math.max(
        10,
        Math.floor(1000 * (1 - timeTaken / (2 * maxTime))),
      );
    }
    ...
    await setDoc(
      doc(db, `liveSessions/${sessionId}/participants`, studentId),
      {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0A0A0B] text-[#E4E4E7] p-8 text-center">
        ...
        <p className="text-xl text-gray-400">
          Olhe para a tela do professor e aguarde o jogo começar.
        </p>
      </div>
    );
  }
  ...
  if (isFinished) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0A0A0B] text-[#E4E4E7] p-8 text-center">
        <h1 id="message" className="text-4xl font-bold mb-4">
          Quiz Finalizado!
        </h1>
        <p className="text-xl">
          Aguarde os resultados finais na tela do professor.
        </p>
      </div>
    );
  }
      ...
      {session.showLeaderboard ? (
        <div className="text-center bg-[#141416] p-8 rounded-2xl border border-[#27272A] max-w-lg w-full">
          <h2 className="text-3xl font-bold mb-4 text-[#D4AF37]">
            Olhe a tela do professor!
          </h2>
          <p className="text-xl text-gray-400">O placar está sendo exibido.</p>
        </div>
      ) : answered ? (
        <div
          className={`p-8 rounded-2xl border border-[#27272A] w-full max-w-lg ${answered.correct ? "bg-green-900 border-green-600" : "bg-red-900 border-red-600"}`}
        >
          {answered.correct ? (
            <>
          ...
          <p className="text-lg text-gray-300 mt-8 opacity-80">
          ...
          {currentQuestion?.type === "match" && (
            <div className="flex flex-col gap-4">
              {currentQuestion.matchPairs.map((pair: any, i: number) => (
                <div
                  key={i}
                  className="flex flex-col md:flex-row gap-4 items-center bg-[#141416] p-4 rounded-xl border border-[#27272A]"
                >
              ...
              <div className="text-center mt-4 text-gray-500 text-sm">
                A resposta é enviada automaticamente ao preencher todos.
              </div>
            </div>
          )}
        </div>
