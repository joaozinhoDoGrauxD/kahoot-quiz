...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      doc(db, "liveSessions", activeSession.pin),
      (doc) => {
    ...
    const unsubParticipants = onSnapshot(
      collection(db, `liveSessions/${activeSession.pin}/participants`),
      (snapshot) => {
  ...
  useEffect(() => {
    if (
      ...
      if (participants.length > 0) {
        const allAnswered = participants.every(
          (p) => p.answeredQuestionIndex === sessionDoc.currentQuestionIndex,
        );
  ...
  const createQuiz = async () => {
    ...
    try {
      ...
      setQuizzes([
        ...quizzes,
        { id: docRef.id, title: quizTitle, questions, teacherId: user.uid },
      ]);
      setQuizTitle("");
      setQuestions([]);
    } catch (e: any) {
    ...
    return (
      <div className="min-h-screen bg-[#0A0A0B] text-[#E4E4E7] p-8 flex flex-col items-center justify-center">
        <div className="max-w-md w-full bg-[#141416] p-10 rounded-2xl border border-[#27272A] text-center shadow-2xl">
          <h1 id="text-professor" className="text-3xl font-black mb-6">
            Acesso do Professor
          </h1>
          <p className="text-gray-400 mb-8">
            Faça login para criar e gerenciar seus quizzes de algoritmos.
          </p>
          <button
            onClick={login}
  ...
  if (activeSession && sessionDoc) {
    const isFinished =
      sessionDoc.status === "active" &&
      sessionDoc.currentQuestionIndex >= (sessionDoc.questions?.length || 0);
    const isWaiting = sessionDoc.status === "waiting";
    const isShowingLeaderboard = sessionDoc.showLeaderboard;
    const currentQuestion =
      sessionDoc.questions?.[sessionDoc.currentQuestionIndex];
    const colors = [
      "bg-red-600",
      "bg-blue-600",
      "bg-yellow-500",
      ...
      <div className="min-h-screen bg-[#0A0A0B] text-[#E4E4E7] p-8 font-sans flex flex-col items-center justify-center">
        {!isWaiting && !isFinished && !isShowingLeaderboard && (
          <div className="absolute top-8 right-12 text-center">
            ...
            <div
              className={`text-6xl font-black ${timeLeft <= 5 ? "text-red-500 animate-pulse" : "text-white"}`}
            >
              {timeLeft}
            </div>
          </div>
            ...
            {isWaiting
              ...
              : isFinished
                ...
                : isShowingLeaderboard
                  ? "PLACAR ATUAL"
                  : `PERGUNTA ${sessionDoc.currentQuestionIndex + 1}`}
          </p>
          ...
          {!isWaiting &&
            ...
            currentQuestion && (
              <div className="w-full">
                <h1 className="text-5xl md:text-6xl font-black mb-16 leading-tight max-w-4xl mx-auto">
                ...
                {currentQuestion.type === "multiple_choice" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left max-w-5xl mx-auto">
                    {currentQuestion.options?.map((opt: any, idx: number) => (
                      <div
                        key={opt.id}
                        className={`${colors[idx % colors.length]} p-6 rounded-xl flex items-center shadow-lg`}
                      >
                        <div className="text-white opacity-80 text-3xl font-black mr-6">
                          {opt.id}
                        </div>
          ...
          <div className="w-full max-w-4xl bg-[#141416] p-8 rounded-xl border border-[#27272A] mb-8 shadow-2xl flex-1 max-h-[60vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              {participants.length === 0 && (
                <p className="text-gray-500 text-center italic text-xl">
                  Aguardando jogadores ingressarem no PIN {activeSession.pin}...
                </p>
              )}
              {participants.map((p, idx) => (
                <div
                  key={p.id}
                  className="flex justify-between items-center bg-[#0A0A0B] p-5 rounded-lg border border-[#27272A] transform transition-transform hover:scale-[1.01]"
                >
                  <div className="flex items-center gap-6">
                    {!isWaiting && (
                      <span
                        className={`font-black text-3xl w-12 text-center ${idx === 0 ? "text-yellow-500" : idx === 1 ? "text-gray-300" : idx === 2 ? "text-amber-600" : "text-gray-600"}`}
                      >
                        #{idx + 1}
                      </span>
                    )}
    ...
    <div className="min-h-screen bg-[#0A0A0B] text-[#E4E4E7] p-8 font-sans">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-[#141416] p-8 rounded-xl border border-[#27272A]">
          <h1 className="text-3xl font-serif italic mb-6 text-[#E4E4E7]">
            Teacher Dashboard
          </h1>
        ...
        <div className="bg-[#141416] p-8 rounded-xl border border-[#27272A]">
          ...
          {quizzes.length === 0 && (
            <p className="text-gray-500 italic">Nenhum quiz criado ainda.</p>
          )}
          <div className="space-y-4">
            {quizzes.map((q) => (
              <div
                key={q.id}
                className="flex justify-between items-center bg-[#0A0A0B] p-4 rounded-lg border border-[#27272A]"
              >
                <div>
